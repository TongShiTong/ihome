...
class Member_EweiShopV2Model
{
	...
	public function getUnionid($unionid = '')
	{
		...
		if (!empty($unionid)) {
			//判断传来的unionid是否为空
			$info = pdo_fetch('select * from ' . tablename('ewei_shop_member') . ' where  unionid=:unionid and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':unionid' => $unionid));
		}
		return $info;
	}
	...
	public function checkMember()
	{
		...
		if (empty($member) && !empty($openid)) {
			$unionid=m('member')->getUnionid($mc['unionid']);
			if(!empty($unionid)){
				$member = array('uniacid' => $_W['uniacid'],'unionid'=>$mc['unionid'],'uid' => $uid, 'openid' => $openid, 'realname' => !empty($mc['realname']) ? $mc['realname'] : '', 'mobile' => !empty($mc['mobile']) ? $mc['mobile'] : '', 'nickname' => !empty($mc['nickname']) ? $mc['nickname'] : '', 'nickname_wechat' => !empty($mc['nickname']) ? $mc['nickname'] : '', 'avatar' => !empty($mc['avatar']) ? $mc['avatar'] : '', 'avatar_wechat' => !empty($mc['avatar']) ? $mc['avatar'] : '', 'gender' => !empty($mc['gender']) ? $mc['gender'] : '-1', 'province' => !empty($mc['resideprovince']) ? $mc['resideprovince'] : '', 'city' => !empty($mc['residecity']) ? $mc['residecity'] : '', 'area' => !empty($mc['residedist']) ? $mc['residedist'] : '', 'createtime' => time(), 'status' => 1);
				pdo_update('ewei_shop_member', $member, array('id' => $unionid['id']));
				...
				foreach ($tables as $k => $v) {
					...
					if (pdo_fieldexists($tablename, 'openid') && pdo_fieldexists($tablename, 'uniacid')) {
						pdo_update($tablename, array('openid' =>$openid), array('uniacid' => $_W['uniacid'], 'openid' => $unionid['openid']));
					}

					if (pdo_fieldexists($tablename, 'openid') && pdo_fieldexists($tablename, 'acid')) {
						pdo_update($tablename, array('openid' => $openid), array('acid' => $_W['acid'], 'openid' => $unionid['openid']));
					}
