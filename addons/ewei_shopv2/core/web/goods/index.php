...
class Index_EweiShopV2Page extends WebPage
{
	...
	//获取硬装软装分类
	public function getRateCategory($data, $pid){
		static $list =array();
		foreach ($data as $key => $value){
			...
			if ($value['parentid'] == $pid){
				...
				//把数组放到list中
				$list[] = $value;
				self::getRateCategory($data, $value['id']);
			}
		}
		return $list;
	}
	//获取品牌分类
	public function getRateCategorybrand($data, $pid){
		static $list =array();
		foreach ($data as $key => $value){
			//第一次遍历,找到父节点为根节点的节点 也就是pid=0的节点
			if ($value['parentid'] == $pid){
				...
				//把数组放到list中
				$list[] = $value;
				self::getRateCategorybrand($data, $value['id']);
			}
		}
		return $list;
	}
	//获取风格分类
	public function getRateCategorystyle($data, $pid){
		static $list =array();
		foreach ($data as $key => $value){
			//第一次遍历,找到父节点为根节点的节点 也就是pid=0的节点
			if ($value['parentid'] == $pid){
				...
				//把数组放到list中
				$list[] = $value;
				self::getRateCategorystyle($data, $value['id']);
			}
		}
		return $list;
	}
	//获取空间分类
	public function getRateCategoryspace($data, $pid){
		static $list =array();
		foreach ($data as $key => $value){
			//第一次遍历,找到父节点为根节点的节点 也就是pid=0的节点
			if ($value['parentid'] == $pid){
				...
				//把数组放到list中
				$list[] = $value;
				self::getRateCategoryspace($data, $value['id']);
			}
		}
		return $list;
	}
