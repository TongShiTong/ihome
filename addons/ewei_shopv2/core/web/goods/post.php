...
$children = array();
$categoryAll = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_category') . ' WHERE uniacid =:uniacid AND enabled=1 ORDER BY parentid ASC, displayorder DESC', array(':uniacid' => $_W['uniacid']));
//获取软装分类
$soft=$this->getRateCategory($category, 1);
//获取硬装分类
$hard=$this->getRateCategory($category, 2);
//获取品牌分类
$brand=self::getRateCategorybrand($category, 4);
//获取风格分类
$style=self::getRateCategorystyle($category, 120);
//获取空间分类
$space=self::getRateCategoryspace($category, 128);
