...
class Index_EweiShopV2Page extends WebPage
{
	...
	public function goodsall(){
		...
		//所有商品
		$goodsall = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid and  deleted=0 ', $paras);
		//上架商品
		$goodshelf = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid  and  deleted=0 and status=1', $paras);
		//下架
		$goodtotals = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid  and  deleted=0 and status=0 ', $paras);
		$goods=array(
			"goodsall"=>$goodsall,
			"goodshelf"=>$goodshelf,
