...
class User_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		foreach ($list as $k=>$v){
			$paras = array(':uniacid' => $_W['uniacid'], ':merchid' => $v['id']);
			//所有商品
			$list[$k]['goodsall']= pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid and merchid = :merchid and  deleted=0 ', $paras);
			//上架商品
			$list[$k]['goodshelf'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid and merchid = :merchid and  deleted=0 and status=1 ', $paras);
			//下架
			$list[$k]['goodtotals'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid and merchid = :merchid and  deleted=0 and status=0 ', $paras);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($_GPC['logo'])) {
				show_json(0, 'logo不能为空');
			}
			if (empty($_GPC['banner'])) {
				show_json(0, 'banner不能为空');
			}
			if (empty($_GPC['desc1'])) {
				show_json(0, '商户简介不能为空');
			}
			...
			$data = array('uniacid' => $_W['uniacid'], 'merchname' => trim($_GPC['merchname']), 'salecate' => trim($_GPC['salecate']), 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'address' => trim($_GPC['address']), 'tel' => trim($_GPC['tel']), 'lng' => $_GPC['map']['lng'], 'lat' => $_GPC['map']['lat'], 'accounttime' => strtotime($_GPC['accounttime']), 'accounttotal' => intval($_GPC['accounttotal']), 'maxgoods' => intval($_GPC['maxgoods']), 'groupid' => intval($_GPC['groupid']), 'cateid' => intval($_GPC['cateid']), 'isrecommand' => intval($_GPC['isrecommand']), 'remark' => trim($_GPC['remark']), 'status' => $status, 'desc' => trim($_GPC['desc1']), 'logo' => save_media($_GPC['logo']),'banner' => save_media($_GPC['banner']), 'payopenid' => trim($_GPC['payopenid']), 'payrate' => trim($_GPC['payrate'], '%'), 'pluginset' => iserializer($_GPC['pluginset']), 'creditrate' => intval($_GPC['creditrate']), 'iscredit' => intval($_GPC['iscredit']), 'iscreditmoney' => intval($_GPC['iscreditmoney']));
