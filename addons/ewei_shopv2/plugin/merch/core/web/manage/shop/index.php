...
class Index_EweiShopV2Page extends MerchWebPage
{
	public function main()
	{
		...
		if (!empty($_W['shopversion'])) {
			if (mcv('shop.adv')) {
				/*header('location: ' . webUrl('shop/adv'));*///设计
	...
	public function goodsAll(){
		...
		$paras = array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']);
		//所有商品
		$goodsall = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid and merchid = :merchid and  deleted=0 ', $paras);
		//上架商品
		$goodshelf = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid and merchid = :merchid and  deleted=0 and status=1 ', $paras);
		//下架
		$goodtotals = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid and merchid = :merchid and  deleted=0 and status=0 ', $paras);
		show_json(1, array('goodsall' => $goodsall,"goodshelf"=>$goodshelf,"goodtotals"=>$goodtotals));
	}
}
