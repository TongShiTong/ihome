...
class Board_EweiShopV2Page extends SnsMobilePage
{
	...
	public function getlist()
	{
		...
		//精华
		foreach ($listbest as &$best) {
		$best['avatar'] = $this->model->getAvatar($best['avatar']);
		$best['createtime'] = $this->model->timeBefore($best['createtime']);
		$best['goodcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_sns_like') . ' where pid=:pid limit 1', array(':pid' => $best['id']));
		$best['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_sns_post') . ' where pid=:pid and deleted = 0 limit 1', array(':pid' => $best['id']));
		$best['content'] = htmlspecialchars_decode($best['content']);
			$images = array();
			$rowimages = iunserializer($best['images']);
			if (is_array($rowimages) && !empty($rowimages)) {
				foreach ($rowimages as $img) {
					if (count($images) <= 2) {
						$images[] = tomedia($img);
		...
		$best['images'] = $images;
		$best['imagewidth'] = '100%';
		$best['imagecount'] = count($rowimages);

			if (count($best['images']) == 2) {
				$best['imagewidth'] = '50%';
			}
			else {
				if (count($best['images']) == 3) {
					$best['imagewidth'] = '33%';
				}
			}

		$best['content'] = $this->model->replaceContent($best['content']);
		}
		//热门
		foreach ($listhot as &$host) {
			$host['avatar'] = $this->model->getAvatar($host['avatar']);
			$host['createtime'] = $this->model->timeBefore($host['createtime']);
			$host['goodcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_sns_like') . ' where pid=:pid limit 1', array(':pid' => $host['id']));
			$host['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_sns_post') . ' where pid=:pid and deleted = 0 limit 1', array(':pid' => $host['id']));
			$host['content'] = htmlspecialchars_decode($host['content']);
			$images = array();
			$rowimages = iunserializer($host['images']);
			if (is_array($rowimages) && !empty($rowimages)) {
				foreach ($rowimages as $img) {
					if (count($images) <= 2) {
						$images[] = tomedia($img);
			...
			$host['images'] = $images;
			$host['imagewidth'] = '100%';
			$host['imagecount'] = count($rowimages);

			if (count($host['images']) == 2) {
				$host['imagewidth'] = '50%';
			}
			else {
				if (count($host['images']) == 3) {
					$host['imagewidth'] = '33%';
				}
			}

			$best['content'] = $this->model->replaceContent($host['content']);
		}
		unset($row);//最新
		unset($host);//热门
		unset($best);//精华
		show_json(1, array('list' => $list,'listbest' => $listbest,'listhot' => $listhot, 'pagesize' => $psize, 'total' => $total));
	}
	...
	public function follow()
	{
		...
		$follow = pdo_fetch('select id from ' . tablename('ewei_shop_sns_board_follow') . ' where unionid=:unionid and openid=:openid limit 1', array(':unionid' => $unionid, ':openid' => $_W['openid']));
		if (!empty($follow)) {
			pdo_delete('ewei_shop_sns_board_follow', array('id' => $follow['id']));
			show_json(1, array('isfollow' => false));
		}
		else {
			$follow = array('uniacid' => $_W['uniacid'],'openid' => $_W['openid'], 'createtime' => time(),'unionid'=>$_GPC['unionid']);
