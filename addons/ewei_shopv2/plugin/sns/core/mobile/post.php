...
class Post_EweiShopV2Page extends SnsMobilePage {
        ...
        public function main() {
                ...
                $member=pdo_fetch('select * from ' . tablename('ewei_shop_member') . ' where uniacid=:uniacid and openid=:openid  limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $post['openid']));
                $isfollow = $this->model->isFollow($member['unionid']);
                $post['unionid']=$member['unionid'];
                $openid=$_W['openid'];
        ...
        public function reserve() {
                ...
                $uniacid = intval($_W['uniacid']);//设计师ID
                include $this->template();
        }
        //当前用户提交预约
        public function designer(){
                ...
                if($_W['ispost']){
                        ...
                        $uid = $_GPC['uid'];
                        $openid = $_W['openid'];
                        $memeber = pdo_fetch("SELECT * FROM ".tablename('ewei_shop_member')." WHERE unionid= :unionid AND uniacid=:uniacid", array(':unionid' => $uid,':uniacid'=>$uniacid));
                        $list=pdo_fetchall("SELECT * FROM ".tablename('ewei_shop_sns_subscribe')." WHERE unionid= :unionid AND uniacid=:uniacid", array(':unionid' => $uid,':uniacid'=>$uniacid));
                        foreach ($list as $k=>$v){
                                $addtime=date('Y-m-d',$v['addtime']);
                                $time=date('Y-m-d',time());
                                if($addtime==$time){
                                        return json_encode(['status'=>0,'msg'=>"用户每天只能预约一次"]);
                                }
                        }
                        $data=array(
                            'uid'=>$memeber['openid'],
                            'designer_id'=>$_GPC['designer_id']?$_GPC['designer_id']:'',
                            'uniacid'=>$uniacid,
                            'province'=>$_GPC['province_city'],
                            'name'=>$_GPC['name'],
                            'measure'=>$_GPC['area'],
                            'layout'=>$_GPC['type'],
                            'house'=>$_GPC['model'],
                            'house_member'=>$_GPC['house_member'],
                            'budget'=>$_GPC['budget'],
                            'decoration_time'=>$_GPC['startingtime'],
                            'check_time'=>$_GPC['checktime'],
                            'phone'=>$_GPC['mobile'],
                            'unionid'=>$_GPC['uid'],
                            'remarks'=>$_GPC['comments'],
                            'addtime'=>time(),
                        );
                        $add=pdo_insert("ewei_shop_sns_subscribe", $data);
                        if($add){
        ...
        public function subscribe(){
                ...
                $uniacid = $_W['uniacid'];
                $openid = $_W['openid'];
                $subscribe = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_sns_subscribe') .' WHERE uniacid = ' . $uniacid . ' AND uid = ' . $openid .' ');
                show_json(1, array('post' => $subscribe));
        }
        //当前用户获取配送状态
        ...
        public function submit() {
                ...
                $data = array(
                    ...
                    'title' => trim($_GPC['title']),
                    'content' => trim($_GPC['content']),
                    'images' => is_array($_GPC['images']) ? iserializer($_GPC['images']) : serialize(array()), 'checked' => $checked);
