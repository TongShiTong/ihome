...
class User_EweiShopV2Page extends SnsMobilePage
{
	...
	public function get_posts()
	{
		...
		$sql = 'select id,title,createtime,content,images,openid ,nickname,avatar,isbest,isboardbest,checked from ' . tablename('ewei_shop_sns_post') . ('  where 1 ' . $condition . ' ORDER BY createtime desc,id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
	...
	public function delete_post(){
		...
		//获取需要删除的id
		$id=$_GPC['id'];
		$list=pdo_update('ewei_shop_sns_post', array('deleted' => 1, 'deletedtime' => time()), array('id' => $id));
		if($list!=false){
			return json_encode(['success'=>1,"msg"=>"删除成功"]);
		}else{
			return json_encode(['success'=>0,"msg"=>"删除失败"]);
		}
	}
