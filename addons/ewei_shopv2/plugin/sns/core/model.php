...
if (!class_exists('SnsModel')) {
	class SnsModel extends PluginModel
	{
         ...
         * @return bool
         */
		public function isFollow($unionid)
		{
			global $_W;
			$count = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_sns_board_follow') . ' where uniacid=:uniacid and unionid=:unionid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':unionid' => $unionid, ':openid' => $_W['openid']));
			return 0 < $count;
		}
...
<a href=\'' . $url . '\'>点击进入查看评论详情</a>';
		...
		{
			return str_replace('
', '<br/>', preg_replace('/\\[EM(\\w+)\\]/', '<img src="'.tomedia('../addons/ewei_shopv2/plugin/sns/static/images/face/${1}.gif').'" class="emoji" />', $content));
		}
