...
class Goods_EweiShopV2Page extends AppMobilePage
{
	...
	public function get_list()
	{
		...
		$args = array('pagesize' => intval($_GPC['pagesize']), 'page' => intval($_GPC['page']), 'isnew' => trim($_GPC['isnew']), 'ishot' => trim($_GPC['ishot']), 'isrecommand' => trim($_GPC['isrecommand']), 'isdiscount' => trim($_GPC['isdiscount']), 'istime' => trim($_GPC['istime']), 'keywords' => trim($_GPC['keywords']), 'cate' => $_GPC['cate'], 'order' => trim($_GPC['order']), 'by' => trim($_GPC['by']), 'from' => 'miniprogram');
	...
	public function get_member_list()
	{
		...
		$groudId=$_GPC['groudid']?$_GPC['groudid']:2;
		//获取商品组信息
		$groud=pdo_fetch('select * from ' . tablename('ewei_shop_goods_group') . ' where id=:id and enabled=1 limit 1', array(':id' => $groudId));
		if(empty($groud)){
			return app_error(-1, '参数错误');
		}
		$args = array('pagesize' => intval($_GPC['pagesize']), 'page' => intval($_GPC['page']),'goodsids'=>$groud['goodsids'],'from' => 'miniprogram');
		$merch_plugin = p('merch');
		$merch_data = m('common')->getPluginset('merch');
		if ($merch_plugin && $merch_data['is_openmerch']) {
			$args['merchid'] = intval($_GPC['merchid']);
	...
	public function get_category()
	{
		...
		//获取软硬装修分类
		$soft=$this->getchildren($category,1,'soft');
		$hard=$this->getchildren($category,2,'hard');
		$hard=$hard+$soft;
		//获取品牌分类
		$brand=$this->getchildren($category, 4,'brand');
		//获取风格分类
		$style=$this->getchildren($category, 120,'style');
		//获取空间分类
		$space=$this->getchildren($category, 128,'space');
		$opencategory = true;
		$plugin_commission = p('commission');
		...
		return app_json(array('allcategory' => $allcategory,"hard"=>$hard,"brand"=>$brand,"style"=>$style,"space"=>$space, 'catlevel' => $catlevel, 'opencategory' => $opencategory));
	}
	public function getchildren($data,$id=0,$table){
		//软硬装修
		if($table=='soft'){
			$children=[];
			foreach ($data as $k=>$v){
				if($v['parentid']==$id){
					$children[$v['id']]=$v;
					unset($data[$k]);
					$children[$v['id']]['children']=self::getchildren($data,$v['id'],$table);
				}
			}
			return $children;
		}elseif($table=='hard'){
