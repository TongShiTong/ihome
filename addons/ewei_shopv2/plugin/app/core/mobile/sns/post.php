...
//            $this->message('未找到版块!');
        ...
        if (!$isSuperManager && !$isManager) {
            ...
            if (is_error($check)) {
                return app_error(888001, $check['message']);
//                show_message($check['message'], '', 'error');
        ...
        if (!$isManager && !$isSuperManager) {
         ...
         $post['sign'] = $member['sign'];
$post['createtime_date']= date("Y-m-d",$post['createtime']);
$post['createtime_datetime']= date("Y-m-d H:i:s",$post['createtime']);
$post['createtime_date2']= date("m-d",$post['createtime']);
$post['replytime_date']= date("Y-m-d",$post['replytime']);
$post['replytime_datetime']= date("Y-m-d H:i:s",$post['replytime']);
$post['replytime_date2']= date("m-d",$post['replytime']);
        $data = array(
            'member' => $member,
            'catelist' => $catelist,
...
//		include $this->template();
    ...
    public function checkPost() {
        ...
        if (empty($post)) {
            return app_error(888001, '该话题或评论不存在');
...
//                show_json(0, $check['message']);
        ...
        if ($len < 3) {
            return app_error(888001, '标题最少3个汉字或字符哦~');
//            show_json(0, '标题最少3个汉字或字符哦~');
        }

        if (25 < $len) {
            return app_error(888001, '标题最多25个汉字或字符哦~');
//            show_json(0, '标题最多25个汉字或字符哦~');
        ...
        if ($len < 3) {
            return app_error(888001, '内容最少3个汉字或字符哦~');
//            show_json(0, '内容最少3个汉字或字符哦~');
        }

        if (1000 < $len) {
            return app_error(888001, '内容最多1000个汉字或字符哦~');
//            show_json(0, '内容最多1000个汉字或字符哦~');
        ...
        if (is_array($_GPC['images'])) {
            ...
            if ($imagesData['imagesnum'] < $imgcount && 0 < $imagesData['imagesnum']) {
                return app_error(888001, '话题图片最多上传' . $imagesData['imagesnum'] . '张！');
//                show_json(0, '话题图片最多上传' . $imagesData['imagesnum'] . '张！');
            }

            if (9 < $imgcount && $imagesData['imagesnum'] == 0) {
                return app_error(888001, '话题图片最多上传9张');
...
//            show_json(0, '您要投诉的话题或评论不存在！');
            return app_error(888001, '您要投诉的话题或评论不存在！');
        }
...
$type=$type?$type:1;
        if (empty($type)) {
//            show_json(0, '请选择投诉类别！');
            return app_error(888001, '请选择投诉类别！');
        }
...
//            show_json(0, '内容最多500个汉字或字符哦~');
            return app_error(888001, '内容最多500个汉字或字符哦~');
        }
...
//            show_json(0, '提交投诉失败，请重试！');
            return app_error(888001, '提交投诉失败，请重试！');
        }
...
//        show_json(1, array('isgood' => $isgood, 'good' => $goodcount));
        $data = array('isgood' => $isgood, 'good' => $goodcount);
        return app_json($data);
    }
...
//            show_json(0, '未找到话题!');
        ...
        if (!$isManager && !$isSuperManager) {
//            show_json(0, '无权设置精华');
            return app_error(888001, '无权设置精华');
        }
...
//            show_json(0, '未找到话题!');
        ...
        if (!$isManager && !$isSuperManager) {
//            show_json(0, '无权设置置顶');
            return app_error(888001, '无权设置置顶');
        }
...
//            show_json(0, '无权设置全站精华');
            return app_error(888001, '无权设置全站精华');
        }
...
//            show_json(0, '无权设置全站置顶');
            return app_error(888001, '无权设置全站置顶');
        }
