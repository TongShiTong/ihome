...
class Board_EweiShopV2Page extends Base_EweiShopV2Page {
    ...
    public function main() {
        ...
        if (empty($id)) {
            ...
            return app_error(888001, '板块ID不能为空');
        }
        ...
        if (!$isManager && !$isSuperManager) {
            ...
            if (is_error($check)) {
                return app_error(888003, $check['message']);
//				show_message($check['message'], mobileUrl('sns'), 'error');
        ...
        $board['logo'] = tomedia($board['logo']);
        $board['banner'] = tomedia($board['banner']);
        $data = [
            'board' => $board,
            'member' => $member,
            'isManager' => $isManager,
...
//				$this->message($check['message'], mobileUrl('sns'));
                return app_error(888002, $check['message']);
            }
        }
    ...
    public function getlist() {
        ...
        $uniacid = $_W['uniacid'];
        $bid = intval($_GPC['bid'])?intval($_GPC['bid']):'';
        ...
        if($type==1){
            //最新
            $sql = 'select id,views,openid,title,createtime,content,images , nickname,avatar,isbest,isboardbest,checked from ' . tablename('ewei_shop_sns_post') . ('  where 1 ' . $condition . ' ORDER BY createtime desc,id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
        }elseif($type==2){
            //热门
            $sql = 'select id,views,openid,title,createtime,content,images , nickname,avatar,isbest,isboardbest,checked from ' . tablename('ewei_shop_sns_post') . ('  where 1 ' . $condition . ' ORDER BY  views desc, id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
        }elseif($type==3){
            //精华
            $sql = 'select id,views,openid,title,createtime,content,images , nickname,avatar,isbest,isboardbest,checked from ' . tablename('ewei_shop_sns_post') . ('  where 1 ' . $condition .' and isbest=1'.' ORDER BY id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
        }elseif($type==4){
            //关注
            //获取自己所关注的用户
            $follow=pdo_fetchall('select f.*,m.openid as openids from ' . tablename('ewei_shop_sns_board_follow')."f"." left join".tablename("ewei_shop_member")." m on m.unionid=f.unionid".' where f.openid=:openid', array(':openid' => $openid));
            //获取所关注用户发布文章
            $condition .=" AND (";
            foreach ($follow as $k=>$v){
                if ($k == 0) {
                    $condition .= 'FIND_IN_SET("' . $v['openids'] . '",openid)';
                }
                else {
                    $condition .= ' OR FIND_IN_SET("' . $v['openids'] . '",openid)';
                }
            }
            $condition .= ' <> 0 )';
            $sql = 'select id,views,openid,title,createtime,content,images , nickname,avatar,isbest,isboardbest,checked from ' . tablename('ewei_shop_sns_post') . ('  where 1 ' . $condition . ' ORDER BY createtime desc,id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
            ...
            $sql = 'select id,views,openid,title,createtime,content,images , nickname,avatar,isbest,isboardbest,checked from ' . tablename('ewei_shop_sns_post') . ('  where 1 ' . $condition . ' ORDER BY createtime desc,id DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize;
            $banner=pdo_fetch('select * from ' . tablename('ewei_shop_sns_board') . ' where id=:bid limit 1', array(':bid' => $bid));
            $banner['banner']=tomedia($banner['banner']);
            $banner['logo']=tomedia($banner['logo']);
        }
        ...
        foreach ($list as $k=>$v){
            $uids=pdo_fetch('select id from ' . tablename('ewei_shop_member') . ' where openid=:openid limit 1', array(':openid' => $v['openid']));
            $list[$k]['uid']=$uids['id'];
            $list[$k]['views']=$v['views']-1;
        }
        unset($row);//最新
        $data = array('list' => $list,'pagesize' => $psize, 'total' => $total,'banner'=>$banner);
        return app_json($data);
    }
    //将数组转为一维数组
...
//		include $this->template();
    ...
    public function get_boardlist() {
        ...
        $data = array('list' => $list, 'pagesize' => $psize, 'total' => $total);
        return app_json($data);
...
//            show_json(1, array('isfollow' => true));
    ...
    public function designer(){
        ...
        $member=pdo_fetch('select * from ' . tablename('ewei_shop_member') . ' where unionid=:unionid and FIND_IN_SET(1,groupid) limit 1',array(':unionid'=>$unionid));
        if($member){
            return app_json(['status'=>1]);
        }else{
            return app_json(['status'=>0]);
        }
